// --- APP VERSION ---
const APP_VERSION = '2026.05.07.04';
...
- 語言判斷觸發詞："中"/"中文"/"중"/"중국어"→zh；"韓"/"韓文"/"한"/"한국어"→ko；"全"/"全部"/"都"/"다"/"all"→both
- "全部"/"전체"→verses設為"all"
- 無法解析→{"error":"無法識別"}
- 無指定語言：中文輸入→zh，韓文→ko
...
async function aiPanelCompose(userId, instruction, input, sendBtn, resultContainer) {
    ...
    if (!instruction) {
        ...
        const hint = currentLang === 'ko' 
            ? `\n\n💡 메시지를 생성하려면 입력:\n@${userId} [지시 사항]\n예: @${userId} 관심사 물어보고 다음 만남 제안`
            : `\n\n💡 要生成訊息，請輸入：\n@${userId} [指示內容]\n例：@${userId} 詢問興趣並提議下次見面`;
        
        const previewItem = { query: `@${userId}`, text: preview + hint, success: true, timestamp: Date.now(), isPreview: true };
