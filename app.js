...
// Firebase 函式（lazy load 後才賦值）
let collection, doc, setDoc, deleteDoc, onSnapshot, updateDoc;

// 只有切換到需要資料的頁面才載入 Firebase
const FIREBASE_PAGES = new Set(['backend','entry','profile','ai-compose','offline-entry','offline-backend']);
...
async function initFirebaseLazy() {
    ...
    _fbInitPromise = (async () => {
        try {
            const [fbApp, fbStore] = await Promise.all([
                import("https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js"),
                import("https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js")
            ]);
            ({ collection, doc, setDoc, deleteDoc, onSnapshot, updateDoc } = fbStore);
            const app = fbApp.initializeApp(firebaseConfig);
            dbInstance = fbStore.getFirestore(app);
            cloudEnabled = true;
...
function _startFirebaseListeners() {
    ...
    onSnapshot(offlineQ, (snapshot) => {
        ...
        const el = document.getElementById('offlineIdInput');
        if (el?.value) window.handleOfflineIdInput();
    });
}
...
// Firebase listeners 在切換到管理頁面時由 initFirebaseLazy() 啟動
...
window.switchPage = (page) => {
    // Firebase Lazy Load：切換到管理頁面才載入
    if (FIREBASE_PAGES.has(page) && !cloudEnabled) {
        initFirebaseLazy();
    }
    const t = i18n[currentLang];
...
// initBackToTop - 非首頁必要，延遲到空閒時初始化
(window.requestIdleCallback || setTimeout)(initBackToTop, 500);
